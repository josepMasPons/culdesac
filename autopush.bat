@echo off
setlocal
 REM   echo "inici autopush "  %date% %time% >>  C:\Users\jmas2\OneDrive\Escriptori\Software\autopushculdesac.log
cd /d C:\Users\jmas2\OneDrive\Escriptori\Software\05 - Culdesac
...
if %errorlevel% equ 0 (
    echo "No hi han canvis "  %date% %time% >>  C:\Users\jmas2\OneDrive\Escriptori\Software\autopushculdesac.log
   REM  echo No hi ha canvis. No es fa cap copia.
    ...
    if errorlevel 1 (
         echo "ERROR no feta la copia : "  %date% %time% >>  C:\Users\jmas2\OneDrive\Escriptori\Software\autopushculdesac.log
        ...
        if errorlevel 1 (
            echo "ERROR el git ha fallat : "  %date% %time% >>  C:\Users\jmas2\OneDrive\Escriptori\Software\autopushculdesac.log
            REM echo ERROR: el git push ha fallat.

            powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$wsh = New-Object -ComObject WScript.Shell; $wsh.Popup('ERROR en la copia a GitHub',5,'GitHub - Autopush',16) | Out-Null"
        ) else (
             echo "COPIA CORRECTA amb data : "  %date% " a les " %time% >>  C:\Users\jmas2\OneDrive\Escriptori\Software\autopushculdesac.log
